...
st.markdown("""
    ...
    /* CLINICAL OUTCOME TIERS (medical traffic-light convention: green / amber / red) */
    ...
    .assessment-box-borderline { background-color: #ffc107 !important; padding: 35px; border-radius: 12px; margin-bottom: 35px; color: #1a1a1a !important; }
...
# =========================================================================
# AUTHENTICATION GATEWAY (unchanged: admin / password)
# =========================================================================
if not st.session_state.logged_in:
    ...
    with col_l2:
        with st.form("login_form", clear_on_submit=False):
            st.markdown(
                "<h2 style='text-align: center; color: #1a3a4b; font-size: 34px; "
                "margin-top: 0; margin-bottom: 30px; font-weight: 800;'>🔒 Secure User Login</h2>",
                unsafe_allow_html=True
            )
...
# =========================================================================
# DATA PREPARATION (runs once the user is authenticated)
# =========================================================================
df = load_data()
...
with col_n4:
    if st.button("📊 DIAGNOSTIC REPORT", key="nav_report", use_container_width=True):
        ...
        st.rerun()
with col_n5:
    if st.button("ℹ️ SYSTEM HELP", key="nav_about", use_container_width=True):
...
if st.session_state.current_nav == "HOME":
    ...
    with col_home_left:
        ...
        <ul style="margin-top: 10px; padding-left: 20px; font-size: 22px;">
            <li><b>Real-Time Patient Analysis</b> &ndash; Processes patient clinical information for immediate risk evaluation.</li>
            <li><b>Validated Machine Learning Models</b> &ndash; Uses multiple machine learning algorithms for accurate heart failure prediction.</li>
            <li><b>Secure User Authentication</b> &ndash; Protects patient records through authenticated system access.</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)
...
# =========================================================================
    ...
    with col_id2:
        patient_id = st.text_input("Hospital ID", value="PT-89422")
    ...
    with col1:
        ...
        creatinine_phosphokinase = st.slider(
            "Creatinine Phosphokinase (CPK) Level (mcg/L)",
            int(df['creatinine_phosphokinase'].min()), int(df['creatinine_phosphokinase'].max()), 250
        )
        diabetes = st.selectbox("Diabetes History", [0, 1], format_func=lambda x: "No" if x == 0 else "Yes")
        ejection_fraction = st.slider(
            "Ejection Fraction (%)",
            int(df['ejection_fraction'].min()), int(df['ejection_fraction'].max()), 25
        )
    ...
    with col2:
        platelets = st.slider(
            ...
            float(df['platelets'].min()), float(df['platelets'].max()), 250000.0, step=1000.0
        )
        rose_creatinine = st.slider(
            "Serum Creatinine Level (mg/dL)",
            float(df['serum_creatinine'].min()), float(df['serum_creatinine'].max()), 2.1, step=0.1
        )
    ...
    # Button label updated per requirements; input-capture logic is unchanged
...
# =========================================================================
# MODULE 3: MACHINE LEARNING MODELS VIEW
# =========================================================================
    ...
    model_choice = st.selectbox(
        "Select Active Machine Learning Analytics Engine",
        ["Random Forest Classifier", "Logistic Regression Framework", "XGBoost Core Engine",
         "Decision Tree Model", "Support Vector Machine (SVM)"]
    )
    ...
    # Fixed validation accuracies as reported in the dissertation
    metrics_data = {
    ...
    # Render as a properly formatted table (no raw HTML), with the
    # best-performing model (Random Forest) highlighted.
    def highlight_best_model(row):
        if row["Classifier Algorithm"] == "Random Forest":
            return ["background-color: #d4edda; font-weight: 700; color: #155724;"] * len(row)
        return [""] * len(row)

    styled_metrics = metrics_df.style.apply(highlight_best_model, axis=1).hide(axis="index")
    st.table(styled_metrics)
    st.caption("Accuracy obtained during the final evaluation and reported in the dissertation. "
               "Random Forest is highlighted as the best-performing, final selected model.")

    # Feature importance chart (unchanged)
    importance_values = model.feature_importances_ if hasattr(model, 'feature_importances_') else np.abs(model.coef_[0])
    feat_imp_df = pd.DataFrame(
        {'Feature Vector': feature_names, 'Weight Importance': importance_values}
    ...
    # Persist the currently trained model/scaler for use on the Report page
    ...
    if st.button("Generate Diagnostic Report", key="run_prediction_btn"):
        if st.session_state.patient_data is None:
            st.warning("Please fill input data form first.")
        else:
            st.session_state.current_nav = "REPORT"
            st.rerun()
...
# =========================================================================
    ...
    if st.session_state.patient_data is not None:
        ...
        # Risk tiers: Low (green) / Moderate (amber) / High (red) — unchanged thresholds
        ...
        metrics_display = {
            "Monitored Clinical Attribute Fields": [
                "Patient Name", "Hospital ID", "Age", "Selected Algorithmic Engine",
                "Ejection Fraction Target Metric", "Serum Creatinine Density Value"
            ],
            "Assigned Patient Vector Values": [
                p['patient_name'], p['patient_id'], f"{p['age']} Years Old", str(st.session_state.model_choice),
                f"{p['ejection_fraction']}% Ratio", f"{p['serum_creatinine']} mg/dL"
            ]
        }
        ...
        st.download_button(
            label="📥 DOWNLOAD PATIENT EVALUATION REPORT",
            data=csv_data,
            file_name=f"Clinical_Report_{p['patient_id']}.csv",
            mime="text/csv"
        )
    else:
        st.warning("No patient data available yet. Please complete the Intake Form and run a prediction first.")
...
# =========================================================================
    ...
    st.markdown(
        "<p style='font-size: 22px; color: #555; margin-bottom: 25px;'>"
        "This guide explains the workflow and functional modules of the Heart Failure Prediction System."
        "</p>",
        unsafe_allow_html=True
    )
